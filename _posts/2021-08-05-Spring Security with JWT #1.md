...
- JWT는 여러 가지 용도로 사용될 수 있다. 그 중 하나는 Bearer Token 이다. 즉, 사용자가 가지고 있기 때문에(당신이 "배어러"가 됨으로써) 어떤 서비스에 액세스할 수 있는 정보를 제공할 수 있다.

- Bearer token 은 여러 가지 방법으로 HTTP 요청에 포함될 수 있으며, 그 중 (아마도 선호되는) 하나는 권한 부여 헤더이다. 그러나 요청 매개 변수, 쿠키 또는 요청 본문에 넣을 수도 있다. 대부분 액세스하려는 서버와 사용자 간에 발생한다.
